...
//	@Autowired
	...
	public AutologinResponse generateAutologinCode(@RequestBody AutologinRequest request) throws Exception {
		...
		if (authenticationManager != null) {
			...
			authenticationManager.authenticate(new AutoLoginAuthenticationRequest(username, password, null));
		}
		logger.info("Autologin authentication request for " + username);
		SocialClientUserDetails user = new SocialClientUserDetails(username, OAuthAuthority.USER_AUTHORITIES);
	...
	public static class AutologinRequestConverter extends AbstractHttpMessageConverter<AutologinRequest> {
		...
		@Override
		protected AutologinRequest readInternal(Class<? extends AutologinRequest> clazz, HttpInputMessage inputMessage) throws IOException,
